...
static bool update_copy_tree(const char* src, const char* dst, char* err, size_t err_sz) {
    struct stat st;
    if (stat(src, &st) != 0) {
        if (err && err_sz) update_safe_snprintf(err, err_sz, "%s", "patching failed");
        return false;
    }
    if (S_ISDIR(st.st_mode)) {
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            update_safe_snprintf(src_path, sizeof(src_path), "%s/%s", src, entry->d_name);
            update_safe_snprintf(dst_path, sizeof(dst_path), "%s/%s", dst, entry->d_name);
...
bool update_extract_zip_dir(const char* zip_path, const char* dest_dir,
    ...
    if (!zip_path || !zip_path[0] || !dest_dir || !dest_dir[0]) {
        if (err && err_sz) update_safe_snprintf(err, err_sz, "%s", "unzip path missing");
        return false;
    }
    if (!update_mkpath(dest_dir, 0755)) {
        if (err && err_sz) update_safe_snprintf(err, err_sz, "%s", "unzip path build failed");
        return false;
    }
    int zip_err = 0;
    zip_t* za = zip_open(zip_path, 0, &zip_err);
    if (!za) {
        if (err && err_sz) update_safe_snprintf(err, err_sz, "%s", "unzip failed");
        return false;
    }
...
bool update_remove_tree(const char* path) {
    if (!path || !path[0]) return false;
    char qpath[1024];
    if (!update_shell_quote(path, qpath, sizeof(qpath))) return false;
    char cmd[1200];
...
bool update_find_single_root(const char* base_dir, char* out, size_t out_sz) {
    if (!base_dir || !base_dir[0] || !out || out_sz == 0) return false;
...
bool update_sync_directories(const char* src, const char* dst, char* err, size_t err_sz) {
    ...
    if (!src || !src[0] || !dst || !dst[0]) {
        if (err && err_sz) update_safe_snprintf(err, err_sz, "%s", "apply path missing");
        return false;
    }
