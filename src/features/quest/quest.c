...
void quest_begin_again(App *a) {
  ...
  /* Mark all current progress as "spent" on the previous quest */
  a->cfg.quest_spent_seconds = a->cfg.focus_total_seconds;
  /* Increment cycle to force a different haiku selection */
  a->cfg.quest_cycle++;
...
static bool quest_pick_daily_pair(const App *a, int date, int season_rank,
  ...
  char buf[64];
  safe_snprintf(buf, sizeof(buf), "%d|%d|%d", date, season_rank,
                a->cfg.quest_cycle);
...
int quest_total_minutes(const App *a, int *out_required) {
  if (out_required) {
    int diff = a->cfg.haiku_difficulty;
    int mins = 45; // Default (Normal)
    if (diff <= 0)
      mins = 30; // Casual
    else if (diff == 1)
      mins = 45; // Normal
  ...
  if (!a)
    return 0;
  uint64_t total = a->cfg.focus_total_seconds;
  if (total < a->cfg.quest_spent_seconds)
    total = a->cfg.quest_spent_seconds; /* Should not happen, but safe guard */
  int focus_minutes = (int)((total - a->cfg.quest_spent_seconds) / 60);
...
#include "quest_render_helpers.inc"
...
void draw_quest(UI *ui, App *a) {
  ...
  if (a->haiku_files.count == 0) {
    ...
    if (a->haiku_left.word_count > 0) {
      reveal_left = (int)floorf((float)a->haiku_left.word_count * progress);
      if (reveal_left > a->haiku_left.word_count)
        reveal_left = a->haiku_left.word_count;
    }
    int reveal_right = 0;
    if (a->haiku_right.word_count > 0) {
      reveal_right = (int)floorf((float)a->haiku_right.word_count * progress);
    ...
    // Poet name: show redacted if not complete, full if complete
    if (a->quest_poet_name[0]) {
      char poet_line[160];
      if (quest_complete) {
        ...
        // Redact
        safe_snprintf(poet_line, sizeof(poet_line), "%s", a->quest_poet_name);
        for (int i = 0; poet_line[i]; i++) {
          if (poet_line[i] != ' ')
            poet_line[i] = '\x1A';
        }
        int w_poet = quest_measure_width(ui->font_small, poet_line);
        int x_poet = (ui->w / 2) - (w_poet / 2);
        quest_draw_line(ui, ui->font_small, x_poet, poet_y, poet_line, main);
      }
    }
    int body_bottom =
    ...
    if (left_text) {
      ...
      while (*p) {
        ...
        line[prefix + len] = 0;
        if (line[0] || len == 0) {
          quest_draw_line(ui, ui->font_small, left_x, y_left, line, accent);
          y_left += line_h;
        }
        if (!nl)
          break;
    ...
    if (right_text) {
      ...
      while (*p) {
        ...
        line[len + suffix] = 0;
        if (line[0] || len == 0) {
          int w = quest_measure_width(ui->font_small, line);
          int x = right_x + col_w - w;
          quest_draw_line(ui, ui->font_small, x, y_right, line, accent);
          y_right += line_h;
        }
        if (!nl)
          break;
