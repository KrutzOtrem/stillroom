...
static void audio_callback(void* userdata, Uint8* stream, int len) {
    ...
    /* Unmute once we have enough buffered audio to avoid underflow crackle.
    ...
    for (int f = 0; f < frames_needed; f++) {
        ...
        /* advance cursors once per frame (SFX only; music ring advances via ring_read_frames above) */
        if (a->sfx.data && a->sfx.pos < a->sfx.frames) {
            a->sfx.pos++;
    ...
    if (!a->music_thread_quit && a->dec.inited) {
        const uint32_t low = a->music_rb.capacity_frames / 4;
